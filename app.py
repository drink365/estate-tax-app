...
st.set_page_config(page_title="《影響力》傳承策略平台 | 整合版", layout="wide", page_icon="assets/logo2.png")
...
# Auth debug (no sidebar)
def _auth_debug_panel(users: dict):
    if os.environ.get("AUTH_DEBUG", "0") != "1":
        return
    with st.expander("🔧 授權診斷（僅在 AUTH_DEBUG=1 時顯示）", expanded=False):
        if not users:
...
def do_login(users: dict):
    ...
    if submitted:
        ...
        st.session_state["username_l"] = username_l
...
with brand_col1:
    try:
        st.image("assets/logo.png", caption=None, use_container_width=True)  # 主頁Logo改為 logo.png
    except Exception:
        pass
with brand_col2:
...
if ensure_auth():
    ...
    with bar_col1:
        st.markdown(f"""<div style="display:flex;align-items:center;gap:0.75rem;font-size:0.9rem;color:#6b7280;">
            <span>歡迎，{name}｜有效期限至 {exp_str}</span></div>""", unsafe_allow_html=True)
    with bar_col2:
