...
SESSION_TTL_SECONDS = int(os.environ.get("SESSION_TTL_SECONDS", "3600"))  # 預設 60 分鐘
ALLOW_TAKEOVER = True  # 允許「搶下使用權」以登出其它裝置

# --------------------------- 授權診斷（可選；無側欄版） ---------------------------
def _auth_debug_panel(users: dict):
    """若設 AUTH_DEBUG=1，於主畫面顯示目前載入到的授權名單（無側欄）。"""
...
def do_login(users: dict):
    _auth_debug_panel(users)  # 無側欄 → 顯示在主畫面
...
with brand_col1:
    try:
        st.image("assets/logo2.png", caption=None, use_container_width=True)  # 與 favicon 一致
    except Exception:
        pass
with brand_col2:
...
# --------------------------- Top Navigation (Tabs) ---------------------------
# 以分頁取代側邊欄：上方兩個分頁
tabs = st.tabs(["🏛️ 遺產稅試算（Estate Tax）", "🎁 保單贈與規劃（CVGift）"])
