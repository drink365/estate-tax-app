...
def run_estate():
    ...
    with col_asset:
        ...
        _ = st.selectbox("適用地區", ["台灣（2025年起）"], index=0, key="region_estate")
    ...
    with colA:
        spouse = st.checkbox("是否有配偶（配偶扣除額 553 萬）", value=False)
        parents = st.number_input(
            "父母數（每人 138 萬，最多 2 人）", min_value=0, max_value=2, value=0
        )
        adult_children = st.number_input(
            "直系血親卑親屬數（每人 56 萬）", min_value=0, max_value=10, value=0
        )
    with colB:
        max_disabled = (1 if spouse else 0) + adult_children + parents
        disabled_people = st.number_input(
            "重度以上身心障礙者數（每人 693 萬）", min_value=0, max_value=max_disabled, value=0
        )
        other_dependents = st.number_input(
            "受撫養之兄弟姊妹、祖父母數（每人 56 萬）", min_value=0, max_value=5, value=0
        )
    ...
    st.markdown("---")
    st.markdown(
        f"### 預估遺產稅：**{int(tax_due):,} 萬**（課稅遺產淨額：{int(taxable_amount):,} 萬）"
    )
    ...
    with col3:
        ...
        st.table(
            pd.DataFrame(
                {"項目": ["課稅遺產淨額", "預估遺產稅"], "金額（萬）": [int(taxable_amount), int(tax_due)]}
            )
        )
    ...
    with c1:
        premium = st.number_input(
            "購買保險保費（萬）", min_value=0, max_value=total_assets, value=default_premium, step=100
        )
    with c2:
        claim = st.number_input(
            "保險理賠金（萬）", min_value=0, max_value=100000, value=default_claim, step=100
        )
    with c3:
        gift = st.number_input(
            "提前贈與金額（萬）",
            min_value=0,
            max_value=max(0, total_assets - premium),
            value=min(default_gift, max(0, total_assets - premium)),
            step=50,
        )
    ...
    # 視覺化（改用新版參數：use_container_width + config）
    fig = px.bar(
        df,
        x="規劃策略",
        y="家人總共取得（萬）",
